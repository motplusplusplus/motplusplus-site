...
import { hotel, type StudioEntry } from "@/lib/studios";
...
function ApplyForm({ studios }: { studios: StudioEntry[] }) {
  const searchParams = useSearchParams();
  const preselectedStudio = searchParams.get("studio") || "";

  // excludes retired/non-current hosts (active:false or hidden:true) so they
  // can't be selected as a studio preference on a live application form.
  const studioOptions = [
...
export default function ApplyFormWithSuspense({ studios }: { studios: StudioEntry[] }) {
  return (
    <Suspense fallback={<div style={{ padding: "64px 24px" }}>loading...</div>}>
      <ApplyForm studios={studios} />
    </Suspense>
  );
}
